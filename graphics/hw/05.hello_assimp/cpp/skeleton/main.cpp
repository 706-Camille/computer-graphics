...
void compose_imgui_frame()
{
  ...
  if (ImGui::Button("Save Scene to output.txt"))
  {
      save_scene();
  }
...
bool save_scene()
{
    ...
    if (!fout)
    {
        std::cerr << "Failed to open or create output.txt for writing.\n";
        return false;
    }
    ...
    for (const auto& obj : g_objects)
    {
        ...
        glm::vec3 trans = obj.get_translate();

        fout << scale.x << " " << scale.y << " " << scale.z << "\n"
    ...
    for (const auto& cam : g_cameras)
    {
        ...
        glm::vec3 at = pos + cam.front_direction();
        glm::vec3 up = cam.up_direction();
